...
export function CanvasExport({
  ...
  const copyRecordField = async (record: DnsRecord, field: 'name' | 'value') => {
    ...
    try {
      await copyText(
        field === 'name'
          ? providerDnsRecordName(record.name, customDomain.dnsZone)
          : record.value,
      )
  ...
  const downloadDnsFile = () => {
    ...
    download(
      `${customDomain.hostname}.dns.txt`,
      dnsZoneFile(customDomain.records, customDomain.dnsZone),
      'text/plain',
    )
  }
  ...
  const copyAllRecords = async () => {
    ...
    try {
      await copyText(
        dnsRecordsClipboardText(customDomain.records, customDomain.dnsZone),
      )
      flashDns('all')
    } catch {
      setError('Clipboard access was blocked. Copy it by hand instead.')
                      ...
                      <div className="space-y-2 border-t pt-3">
                        ...
                        {customDomain ? (
                          <div className="space-y-2 rounded-md border bg-background p-2.5">
                            ...
                            customDomain.status !== 'active' ? (
                              <div className="space-y-2 border-t pt-2">
                                <div className="flex items-center justify-between gap-2">
                                  <p className="text-2xs text-muted-foreground">
                                    Add these records in the{' '}
                                    <span className="font-mono">
                                      {customDomain.dnsZone}
                                    </span>{' '}
                                    DNS zone.
                                  </p>
                                ...
                                {customDomain.records
                                  ...
                                  .map((record) => {
                                    ...
                                    const providerName = providerDnsRecordName(
                                      record.name,
                                      customDomain.dnsZone,
                                    )
                                    return (
                          ...
                          <div className="space-y-1.5">
                            ...
                            <p className="text-2xs text-muted-foreground">
                              Use a subdomain such as www.example.com. Root domains
                              are not supported yet. After connecting, add the DNS
                              records shown here at your provider.
                            </p>
                          </div>
                        )}
                      </div>
