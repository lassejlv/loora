...
export async function customDomainSiteResponse(request: Request) {
...
export const Route = createFileRoute('/api/custom-domain-site')({
  server: {
    handlers: {
      GET: ({ request }) => customDomainSiteResponse(request),
      HEAD: async ({ request }) => {
        const response = await customDomainSiteResponse(request)
