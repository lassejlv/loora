...
import { ChevronRightIcon, XIcon } from "../icons/index.tsx";
          ...
          className={cn(
            ...
            variant !== "straight" &&
              cn(
                ...
                position === "top" &&
                  "rounded-b-lg **:data-[slot=drawer-footer]:rounded-b-[calc(var(--radius-lg)-1px)]",
                position === "left" &&
                  "rounded-e-lg **:data-[slot=drawer-footer]:rounded-ee-[calc(var(--radius-lg)-1px)]",
                position === "right" &&
                  "rounded-s-lg **:data-[slot=drawer-footer]:rounded-es-[calc(var(--radius-lg)-1px)]",
              ),
            variant === "default" &&
              cn(
                position === "bottom" &&
                ...
                position === "left" &&
                  "before:rounded-e-[calc(var(--radius-lg)-1px)]",
                position === "right" &&
                  "before:rounded-s-[calc(var(--radius-lg)-1px)]",
              ),
            variant === "inset" &&
              "before:hidden sm:rounded-lg sm:border sm:after:bg-transparent sm:**:data-[slot=drawer-footer]:rounded-b-[calc(var(--radius-lg)-1px)]",
...
export function DrawerHeader({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col gap-1.5 p-4 in-[[data-slot=drawer-popup]:has([data-slot=drawer-panel])]:pb-2 max-sm:pb-3",
...
export function DrawerFooter({
  ...
  const defaultProps = {
    className: cn(
      "flex flex-col-reverse gap-1.5 px-4 pb-(--safe-area-inset-bottom,0px) sm:flex-row sm:justify-end",
      !allowSelection && "cursor-default",
      variant === "default" &&
        "border-t bg-muted/45 pt-3 pb-[calc(env(safe-area-inset-bottom,0px)+--spacing(3))]",
      variant === "bare" &&
        "in-[[data-slot=drawer-popup]:has([data-slot=drawer-panel])]:pt-2 pt-3 pb-[calc(env(safe-area-inset-bottom,0px)+--spacing(4))]",
      className,
    ),
...
export function DrawerMenuItem({
  ...
  const defaultProps = {
    className: cn(
      "flex min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-foreground outline-none hover:bg-accent hover:text-accent-foreground disabled:pointer-events-none disabled:opacity-64 data-[variant=destructive]:text-destructive-foreground sm:min-h-7 sm:text-xs [&>svg:not([class*='opacity-'])]:opacity-80 [&>svg:not([class*='size-'])]:size-4.5 sm:[&>svg:not([class*='size-'])]:size-3.5 [&>svg]:pointer-events-none [&>svg]:-mx-0.5 [&>svg]:shrink-0",
      className,
    ),
...
export function DrawerMenuTrigger({
  ...
  return (
    <DrawerTrigger
      className={cn(
        "flex min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-foreground outline-none hover:bg-accent hover:text-accent-foreground sm:min-h-7 sm:text-xs [&_svg:not(:last-child)]:-mx-0.5 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
...
export function DrawerMenuCheckboxItem({
  ...
  return (
    <CheckboxPrimitive.Root
      checked={checked}
      className={cn(
        "grid min-h-9 w-full cursor-default select-none items-center gap-2 rounded-sm px-2 py-1 text-base text-foreground outline-none hover:bg-accent hover:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-64 sm:min-h-7 sm:text-xs [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-3.5 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0",
        variant === "switch"
