...
Env (see README): `DATABASE_URL`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`, `REQUIRE_PREVIEW_ACCESS`, `WAFER_API_KEY`, `LWC_SECRET`, `CODEX_CLIENT_VERSION`, optional `S3_*`, optional Google OAuth pair, optional `VITE_DATABUDDY_CLIENT_ID` (Databuddy analytics; build-time, inlined into the client bundle — must be set in the Railway build env, unset skips analytics).
