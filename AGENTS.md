...
- **Linux embedded canvas works on X11 / XWayland.** GPUI exposes `RawWindowHandle::Xcb`; wry’s `build_as_child` only matches `Xlib`. The app remaps Xcb→Xlib (same XID), forces `GDK_BACKEND=x11`, inits GTK, and pumps `gtk::main_iteration_do` from the canvas paint path. When both `WAYLAND_DISPLAY` and `DISPLAY` are set, Loora prefers X11/XWayland for embed (clears `WAYLAND_DISPLAY` unless `LOORA_NATIVE_WAYLAND=1`). Pure Wayland (no `DISPLAY`) still soft-fails — true native Wayland child embed needs a GTK-owned host window, which GPUI does not provide. The native webview is hidden (collapsed/unmapped + GTK pump) on `/settings…` and while GPUI overlays are open (command palette, image picker, color picker, inspector enum menus) so it cannot paint over chrome.
- **Validate on Linux via build + lint + tests, plus GUI on `DISPLAY=:1`.** `just check`, `cargo clippy --workspace --all-targets`, `cargo build -p loora-desktop --locked`, and `cargo test -p loora-engine -p gpui-router -p loora-ui --locked`. Routes: `/` canvas; `/settings`, `/settings/appearance`, `/settings/shortcuts` (open with `Ctrl+,` / titlebar gear; **← Back** returns to canvas). The `loora-ui` tests also run headless via `gpui` `test-support`.
...
- **Pre-existing fmt noise (do not "fix" as product work):** `cargo fmt --all -- --check` may report diffs in `crates/engine/src/camera.rs` / `canvas_import.rs` under current rustfmt.
