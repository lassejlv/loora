...
type ExportFormat = 'png' | 'html' | 'react' | 'json' | 'handoff'
...
const FORMATS: {
  ...
  },
  {
    ...
    description: 'The same markup as a component with scoped CSS',
  },
  {
    id: 'json',
    extension: 'JSON',
 ...
 */
function subtreeNodes(document: CanvasDocumentV2, roots: CanvasNode[]) {
...
function ancestorPage(document: CanvasDocumentV2, nodeId: NodeId | undefined) {
...
function imageSources(document: CanvasDocumentV2, roots: CanvasNode[]) {
  return [
    ...new Set(
...
function ScopeButton({
  ...
  return (
    <button
      ...
      title={title}
      className={cn(
        'h-6 flex-1 rounded text-[11px] text-muted-foreground',
...
export function CanvasV2Export({
  controller,
  open,
  onOpenChange,
}: {
      ...
      return {
        text:
          format === 'html'
            ...
            : compileReactComponent(document, options),
        error: null,
      }
    } catch (cause) {
      return {
  ...
  const previewHtml = useMemo(() => {
    if (!open || (format !== 'html' && format !== 'react')) return null
    try {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogPopup className="max-w-5xl p-0" bottomStickOnMobile={false}>
        <DialogHeader className="border-b px-4 py-3">
          ...
          <DialogDescription>
            Everything here is generated from the Canvas document. Exports never
            come back in.
          </DialogDescription>
        </DialogHeader>

        <div className="grid h-[min(78svh,42rem)] grid-cols-[minmax(0,17rem)_minmax(0,1fr)]">
          <div className="flex min-h-0 flex-col overflow-y-auto border-e">
            <div className="p-1">
              {FORMATS.map((item) => (
                ...
                >
                  <span className="w-9 shrink-0 font-mono text-[10px] font-medium text-muted-foreground">
                    {item.extension}
                  </span>
              ...
              <Row label="What">
                ...
                {scoped && scope === 'page' && pages.length > 1 ? (
                  <select
                    aria-label="Page"
                    value={pageId ?? ''}
                    className="h-7 w-full rounded-md border bg-background px-1.5 text-[11px] outline-none"
          ...
          <div className="flex min-h-0 flex-col">
            ...
            <div
              ref={preview.ref}
              className="min-h-0 flex-1 overflow-auto bg-cx-canvas"
            >
              {format === 'handoff' ? (
                  ...
                  {handoff ? (
                    <>
                      <textarea
                        ...
                        className="min-h-40 w-full resize-none rounded-lg border bg-background p-3 font-mono text-[11px] leading-relaxed outline-none"
                ...
                <pre className="h-full overflow-auto bg-muted p-3 text-[11px] leading-5">
