...
- **The GUI cannot run on Linux (X11 or Wayland).** The app builds and GPUI opens a window, but it panics in `CanvasWorkspace::new` (`crates/ui/src/canvas/workspace.rs`) creating the embedded WebKit canvas: `create canvas webview: the window handle kind is not supported`. Root cause: `wry`'s Linux child-webview (`build_as_child`) only accepts a `RawWindowHandle::Xlib`, but GPUI's Linux backend always exposes `Xcb` (X11) or `Wayland`. This is a code-level cross-platform gap, not an environment problem — the app is intended for macOS (WKWebView). Do not expect `just dev` / `just run` to open a usable window here.
- **Validate on Linux via build + lint + tests instead of the GUI:** `just check` (type-check), `cargo clippy --workspace --all-targets` (passes with warnings only), `cargo build -p loora-desktop --locked`, and `cargo test -p loora-engine -p gpui-router -p loora-ui --locked`. The `loora-ui` tests run headless via `gpui` `test-support` (no display needed) and exercise the canvas/webview logic without instantiating a real webview.
