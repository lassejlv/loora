...
- **Pull requests** (`packages/rpc/src/pull-requests.ts`, `design_pull_request` + `design_pull_request_comment`): a branch opened for review. The row id is the review URL's capability token, like `publish_link` — `/pr/:id` is public, `/api/pr/:id` serves the payload (Main vs branch shapes with asset URLs rewritten to `/api/pr/:id/asset/:id`, commits from `designVersion`, thread) and takes guest comments; a signed-in viewer comments as their account, never as a typed name. Open branches stay editable while under review (`proposed` is not read-only), and `draft.apply` marks any open PR merged inside its transaction. Public reads bill against the same `publishEgress` cap.
