...
| AI (Loora-managed) | Neon AI Gateway | Model inference for included AI credits |
...
TODO(confirm): exact primary hosting and subprocessors’ processing regions for Railway, S3-compatible asset storage, Neon AI Gateway, Databuddy, Polar, and Google/GitHub/Figma/OpenAI, and the transfer safeguards relied on (for example EU Standard Contractual Clauses or adequacy decisions).
...
- **Loora-managed models** are routed through our AI provider (currently Neon AI Gateway). Provider credentials stay on our servers; they are not exposed to the browser.
...
Neon's data-handling terms and the gateway's upstream provider details should be confirmed before making retention or model-training claims.
