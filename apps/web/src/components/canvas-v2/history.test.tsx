...
const list = mock()
const compareV2 = mock()
const commitV2 = mock()
const restoreV2 = mock()

mock.module('#/lib/orpc-client', () => ({
  orpc: { history: { list, compareV2, commitV2, restoreV2 } },
}))
mock.module('#/lib/canvas-v2-migration', () => ({
  migrateCanvasVersion: mock(async () => undefined),
}))

const { CanvasV2History } = await import('./history')
...
function setup() {
  ...
  const view = render(
    <CanvasProvider engine={engine}>
      ...
      <CanvasV2History controller={controller as never} readOnly={false} />
    </CanvasProvider>,
  )
...
describe('CanvasV2History', () => {
  beforeEach(() => {
    ...
    commitV2.mockReset().mockResolvedValue({ id: 'v2' })
    restoreV2.mockReset().mockResolvedValue({ document: fixture('Restored'), revision: 8 })
  })
  ...
  test('lists checkpoints and selects the newest one', async () => {
    ...
    await waitFor(() =>
      expect(compareV2).toHaveBeenCalledWith({
        designId: 'design-1',
        draftId: null,
        id: 'v1',
      }),
  ...
  test('names a checkpoint instead of saving them all as "Manual checkpoint"', async () => {
    ...
    await waitFor(() => expect(commitV2).toHaveBeenCalledTimes(1))
    expect((commitV2.mock.calls[0]![0] as { message: string }).message).toBe(
      'Before the rewrite',
    )
  })
  ...
  test('checkpoints the current state before replacing it', async () => {
    ...
    await waitFor(() => expect(restoreV2).toHaveBeenCalledTimes(1))
    expect(commitV2).toHaveBeenCalledTimes(1)
    expect((commitV2.mock.calls[0]![0] as { message: string }).message).toBe(
      'Before restore',
    )
  ...
  test('says a V1 checkpoint converts on restore rather than previewing it', async () => {
    ...
    expect(await view.findByText(/predates Canvas V2/)).toBeTruthy()
    expect(view.getByRole('button', { name: /Convert and restore/ })).toBeTruthy()
    expect(compareV2).not.toHaveBeenCalled()
  })
})
