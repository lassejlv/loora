...
- **Linux embedded canvas works on X11.** GPUI exposes `RawWindowHandle::Xcb`; wry’s `build_as_child` only matches `Xlib`. The app remaps Xcb→Xlib (same XID) via a local `HasWindowHandle` adapter, forces `GDK_BACKEND=x11`, calls `gtk::init()` once, and pumps `gtk::main_iteration_do` from the canvas paint path. On X11 + lavapipe this VM can run the interactive HTML canvas. Wayland-native child embedding is still unsupported — soft-fail (log + placeholder) remains for non-X11 sessions.
