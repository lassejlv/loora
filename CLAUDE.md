...
- **Inline editing (`textEditable`).** In edit mode the frame intercepts clicks: text-bearing nodes become contenteditable (Enter/blur commits, Escape restores) and post before/after text pairs (`loora:text-edit`); `<img>` clicks post their raw `src` attribute (`loora:image-pick`, inlined data URLs reverse-mapped to `/api/asset/…` via `sourceUrlForInlinedSrc`); right-clicks post the node's tag + class value (`loora:style-pick`). The frame NEVER rewrites code — callers map reports onto the source with `applyTextEdits` / `replaceImageSource` / `replaceClassValue` (lib/canvas.ts, all exact search/replace; class values are quote-anchored so `"flex"` never corrupts `flex-col`). Unmappable edits (repeated, generated, or whitespace-collapsed text) fail loudly and callers revert the frame with a key bump. `StyleEditorPanel` + `lib/style-edit.ts` do the Tailwind token surgery (classifiers keep `text-xl` apart from `text-red-500`); `ImagePickerDialog` wraps `AssetsPanel`. Wired in both the canvas (double-click = interact + editing on; the interact tool `i` is pure play mode; label chip toggles) and `/blockpage` ("Edit text" toggle).
