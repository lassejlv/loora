...
One typed catalog: `packages/agent/src/models.ts`. Providers are either `openrouter` (label, `apiKeyEnv`) or `chatgpt`. `mini` uses the unpinned `openrouter/free` router at zero Loora usage cost; `max` uses OpenRouter's `z-ai/glm-5.2` slug and the `wafer/fp4` endpoint with provider fallbacks disabled. The OpenRouter workspace must have a prioritized Wafer BYOK key with **Always use for this provider** enabled to prevent fallback to shared capacity for Max. ChatGPT-backed models proxy through the user's **own** connected ChatGPT account (`@opencoredev/loginwithchatgpt-*`, `packages/agent/src/internal/chatgpt-auth.ts`, `/api/chatgpt/*`) and are listed only when that account reports them available — they bypass the app's spend limits because the user pays. Provider credentials never reach the browser. `supportsImageInput` per model gates snapshots and `viewCanvas`; `packages/agent/src/messages.ts` strips image parts for models without it. Adding a provider/model: see README.
